...
const generarHeatmap = async (cicloId, fechaInicio, fechaFin) => {
    ...
    while (baseDate <= fechaFin && i < 100) {
        ...
        if (baseDate.getDay() !== 0 && baseDate.getDay() !== 6) {
            ...
            for (const tarea of tareas) {
                ...
                if (tarea.periodicidad === 'DIARIA')
                    appliesToThisDay = true;
                if (tarea.periodicidad === 'SEMANAL' && baseDate.getDay() === 5)
                    appliesToThisDay = true;
                if (appliesToThisDay) {
                    total++;
                }
...
exports.generarHeatmap = generarHeatmap;
const calcularAnalisisTareas = async (cicloId, fechaInicio, fechaFin) => {
    const tareasBD = await db_1.default.tarea.findMany({
        where: { cicloId },
        include: { cumplimientos: true }
    });
    ...
    const analisis = tareas.map(t => {
        ...
        while (pDate <= endD) {
            ...
            if (!isWknd) {
                if (t.periodicidad === 'DIARIA')
                    cicloTotalProyectado++;
                if (t.periodicidad === 'SEMANAL' && isFri)
                    cicloTotalProyectado++;
            }
            pDate.setDate(pDate.getDate() + 1);
        }
        let historialEjecuciones = [];
        ...
        const tendencia = Array(5 - last5.length).fill('bg-gray-200').concat(last5.map(h => {
            if (h.completada)
                return 'bg-[#A9D42C]';
            if (h.isToday && !h.completada)
                return 'bg-gray-200';
            return 'bg-red-500';
        }));
...
exports.calcularAnalisisTareas = calcularAnalisisTareas;
const extraerSabidurias = async (cicloId) => {
    const tareas = await db_1.default.tarea.findMany({
        where: { cicloId },
        include: {
            cumplimientos: {
...
exports.extraerSabidurias = extraerSabidurias;
const calcularMedallas = async (user, cicloInfo) => {
...
const obtenerDashboardBff = async (userId) => {
    ...
    let heatmapInfo = { heatmapDays: [], porcentajeCompromiso: 0 };
    let analisisTareas = [];
    let sabidurias = [];
    let diasHabiles = 0;
    if (activeCiclo) {
        ...
        activeCiclo.diaHabilActual = diasHabiles;
        heatmapInfo = await (0, exports.generarHeatmap)(activeCiclo.id, activeCiclo.fechaInicio, activeCiclo.fechaFin);
    ...
    const nivelDetalle = (0, gamificationService_1.calcularNivel)(user.xpTotal || 0);
